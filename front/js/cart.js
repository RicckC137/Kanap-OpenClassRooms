...
async function fetchPrice() {
  try {
    ...
    const kanaps = await Promise.all(
      kanapFromStorage.map(async (kanap) => {
    ...
    totalQuantity.textContent = kanaps.reduce(function (
      previousValue,
      currentValue
    ) {
  ...
  function deleteButton() {
    document.querySelectorAll(".deleteItem").forEach((element) => {
      element.addEventListener("click", (e) => {
        ...
        const found = parseKanap.find(
          (element) => element.id === idi && element.color === colori
        );
  ...
  function submit() {
    document.getElementById("order").addEventListener("click", (e) => {
      ...
      if (totalInQuantity <= 0) {
        alert(1);
        alert(
          "Quantité du panier insuffisante, merci d'ajouter un produit à votre commande"
        );
      } else if (
        // Validation des éléments saisis dans le formulaire par l'utilisateur
        ...
        fetch("http://localhost:3000/api/products/order", {
          ...
          .then((data) => {
            // Si ok, envoi de la commande avec ID-order
